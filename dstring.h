...
/**
 ...
 DSLOCAL(S)        -- create a localdstring that destroys itself 
                      at the end of the function scope
                      
 DSLOCAL_FMT(fmt, ...) -- DSLOCAL with a printf-style format string.                       
...
#define DSVERSION "1.7 02 Jan 2016" 
...
_INLINE_ dstring dstring_version(void);
_INLINE_ dstring dstring_version(void){return _dstring_version;}
/*************************************************/
...
_INLINE_ dstptr __dststruct (const dstring ds); // not used but here for completeness

_INLINE_ size_t dst_mem          (const dstring ds); // memory size
_INLINE_ size_t dstlen           (const dstring ds); // string length
_INLINE_ size_t dstfree          (const dstring ds); // free   length
_INLINE_ int    dstcheck         (const dstring ds); // integrity 
_INLINE_ int    is_dstring       (const dstring ds); // type
_INLINE_ int    is_blank         (const dstring ds);  // "" check 
_INLINE_ int    is_empty         (cstring str);       // emptystring check

/*************************************************/

/* [internal] return dst_struct of a ds */
_INLINE_ dstptr __dststruct( const dstring ds)
{
  DSTPTR(ds);
  return d;
}
...
_INLINE_ size_t dst_mem(const dstring ds)
{
  if (!ds) return 0;
  DSTPTR(ds);
...
_INLINE_ size_t dstlen(const dstring ds)
{
  if (!ds) return 0;
  DSTPTR(ds);
  return d->len;
...
_INLINE_ size_t dstfree(const dstring ds) 
{
  if (!ds) return 0;
  DSTPTR(ds);
  return d->free;
...
_INLINE_ int dstcheck( const dstring ds)
{
...
_INLINE_ int is_blank ( const dstring ds )
{
...
_INLINE_ int is_empty( cstring str )
{
